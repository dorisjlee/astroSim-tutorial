...
def plot_dens(i,fname="sphere",plane="z", velocity=False,grid=False,zmin ="",zmax="",magnetic=False, particle=False,zoom=""):
    ...
    slc.save(SAVE_PATH+"{0}_{1}_zoom_4_nofixedcolor.png".format(ds,physical_quantity))
...
def all_direction_slices(i,fname="sphere",physical_quantity="density",zmin="",zmax="",zoom=""):
	...
	if zoom!="": slc.zoom(zoom)
        ...
        slc._setup_plots()
    plt.savefig(SAVE_PATH+"{0}_alldir_{1}_zoom_4_nofixedcolor.png".format(ds,physical_quantity))
START_TIME = 0
END_TIME = 494
...
plot_dens(END_TIME,velocity=True,particle=True,zoom=4)
all_direction_slices(END_TIME,zoom=4)
